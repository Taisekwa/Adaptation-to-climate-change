...
##Test plot
deep_south$option<-  factor(deep_south$option, levels=c("current","optimistic","less_optimistic")) # Ordering for the legend.
ggplot()+        
  ...
  scale_fill_discrete(breaks=c('current', 'optimistic', 'less_optimistic')) + 
...
filter(dacade %in% c("2010_2020", "2040_2050" )) %>% filter(option %in% c("current" , "optimistic")) %>% filter(pathway %in% c("kurrent","rcp4.5"))
...
temp1 <- deep_south %>% 
  select(dacade, pathway, option, pgr, month )%>%
  filter(dacade %in% c("2010_2020", "2040_2050" )) %>% filter(option %in% c("current" , "less_optimistic")) %>% filter(pathway %in% c("kurrent","rcp4.5"))

# filter late century optimistic
temp2 <- deep_south %>% 
  select(dacade, pathway, option, pgr, month )%>%
  filter(dacade %in% c("2010_2020", "2090_2100" )) %>% filter(option %in% c("current" , "optimistic")) %>% filter(pathway %in% c("kurrent","rcp4.5"))

# filter late century less optimistic
temp3 <- deep_south %>% 
  select(dacade, pathway, option, pgr, month )%>%
  filter(dacade %in% c("2010_2020", "2090_2100" )) %>% filter(option %in% c("current" , "less_optimistic")) %>% filter(pathway %in% c("kurrent","rcp4.5"))
...
temp$option<-  factor(temp$option, levels=c("current","optimistic","less_optimistic")) # Ordering for the legend.
...
ggplot(data = temp, mapping = aes(y = pgr, x = month, group=option, color=option)) +
 ...
 geom_point(aes(color=option), size=2)+  
  ...
  ggtitle(" Figure 1, RCP4.5 predicted pasture growth rates") +
...
temp4 <- deep_south %>% 
  select(dacade, pathway, option, pgr, month )%>%
  filter(dacade %in% c("2010_2020", "2040_2050" )) %>% filter(option %in% c("current" , "optimistic")) %>% filter(pathway %in% c("kurrent","rcp8.5"))
...
temp5 <- deep_south %>% 
  select(dacade, pathway, option, pgr, month )%>%
  filter(dacade %in% c("2010_2020", "2040_2050" )) %>% filter(option %in% c("current" , "less_optimistic")) %>% filter(pathway %in% c("kurrent","rcp8.5"))

# filter late century optimistic
temp6 <- deep_south %>% 
  select(dacade, pathway, option, pgr, month )%>%
  filter(dacade %in% c("2010_2020", "2090_2100" )) %>% filter(option %in% c("current" , "optimistic")) %>% filter(pathway %in% c("kurrent","rcp8.5"))

# filter late century less optimistic
temp7 <- deep_south %>% 
  select(dacade, pathway, option, pgr, month )%>%
  filter(dacade %in% c("2010_2020", "2090_2100" )) %>% filter(option %in% c("current" , "less_optimistic")) %>% filter(pathway %in% c("kurrent","rcp8.5"))
...
tempz$option<-  factor(tempz$option, levels=c("current","optimistic","less_optimistic")) # Ordering for the legend.
...
ggplot(data = tempz, mapping = aes(y = pgr, x = month, group=option, color=option)) +
  ...
  facet_wrap (~Option, scales = "fixed") + theme_grey() +
  ggtitle("Figure 2, RCP8.5 predicted pasture growth rates") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  guides(color = guide_legend(title = "Outlook"))
